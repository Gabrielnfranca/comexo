...
export default function NovoClientePage() {
  ...
  // Campos auto-preenchíveis via CNPJ
  ...
  async function consultarCnpj() {
    ...
    if (limpo.length !== 14) { setCnpjMsg({ tipo: 'erro', texto: 'CNPJ deve ter 14 dígitos' }); return }
    setBuscandoCnpj(true); setCnpjMsg(null)
    try {
      const resp = await fetch(`/api/cnpj/${limpo}`)
      const data = await resp.json()
      if (!resp.ok) { setCnpjMsg({ tipo: 'erro', texto: data.erro ?? 'Erro na consulta' }); return }
      ...
      setEstadoUF(data.estado ?? '')
      setCep(data.cep ?? '')
      setCnpjMsg({ tipo: 'ok', texto: `Dados carregados${data.situacao ? ` — Situação: ${data.situacao}` : ''}` })
    } catch {
      setCnpjMsg({ tipo: 'erro', texto: 'Falha ao consultar. Verifique sua conexão.' })
    } finally {
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-4">
        <Link
          ...
          className="p-2 text-slate-500 dark:text-slate-400 hover:text-white hover:bg-slate-900/5 dark:bg-white/5 rounded-xl transition"
        >
          <ArrowLeft size={20} />
        </Link>
        <div>
          <h1 className="text-xl font-bold text-white">Novo Cliente</h1>
          <p className="text-slate-500 dark:text-slate-400 text-sm">Cadastrar importador ou exportador</p>
        </div>
      </div>
      ...
      <form action={action} className="space-y-5">
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/5 rounded-2xl p-5">
          ...
          <div className="grid grid-cols-1 gap-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-slate-500 dark:text-slate-400 text-xs mb-1.5">Nome do Contato</label>
                <input
                  name="contato_nome"
              ...
              <div>
                <label className="block text-slate-500 dark:text-slate-400 text-xs mb-1.5">Telefone</label>
                <input
                  name="telefone"
                  value={telefone}
            ...
            <div>
              <label className="block text-slate-500 dark:text-slate-400 text-xs mb-1.5">E-mail</label>
              <input
                name="email"
                type="email"
                value={email}
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/5 rounded-2xl p-5">
          ...
          <div className="grid grid-cols-1 gap-4">
            <div>
              <label className="block text-slate-500 dark:text-slate-400 text-xs mb-1.5">Endereço</label>
              <input
                name="endereco"
                value={endereco}
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/5 rounded-2xl p-5">
          ...
          <div className="grid grid-cols-1 gap-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-slate-500 dark:text-slate-400 text-xs mb-1.5">Número RADAR</label>
                <input
                  name="numero_radar"
                  placeholder="Ex: 10.000.000/0001-00"
                  className="w-full bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-white/10 rounded-xl px-4 py-2.5 text-white text-sm placeholder-slate-600 font-mono
                             focus:outline-none focus:border-amber-500/50 transition"
                />
              </div>
              <div>
                <label className="block text-slate-500 dark:text-slate-400 text-xs mb-1.5">Tipo de Habilitação</label>
                <select
                  name="tipo_habilitacao_radar"
                  className="w-full bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-white/10 rounded-xl px-4 py-2.5 text-white text-sm
                             focus:outline-none focus:border-amber-500/50 transition"
                >
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-slate-500 dark:text-slate-400 text-xs mb-1.5">Validade RADAR</label>
                <input
                  name="validade_radar"
                  type="date"
                  className="w-full bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-white/10 rounded-xl px-4 py-2.5 text-white text-sm
              ...
              <div>
                <label className="block text-slate-500 dark:text-slate-400 text-xs mb-1.5">Regime Tributário</label>
                <select
            ...
            <div>
              <label className="block text-slate-500 dark:text-slate-400 text-xs mb-1.5">Inscrição Estadual</label>
              <input
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/5 rounded-2xl p-5">
          ...
          <textarea
            ...
            className="w-full bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-white/10 rounded-xl px-4 py-2.5 text-white text-sm placeholder-slate-600
        ...
        <div className="flex gap-3 justify-end">
          <Link
            href="/dashboard/clientes"
            className="px-5 py-2.5 text-slate-500 dark:text-slate-400 hover:text-white border border-slate-200 dark:border-white/10 hover:border-white/20 rounded-xl text-sm transition"
          >
            Cancelar
          </Link>
          <button
